...
Betrachte ein System im nicht-Gleichgewicht, das equilibriert (irreversibel)  %TODO image
Beispielsweise ein Gas, das mit der Zeit sinkt oder ein Spinsystem. (Letzteres equilibriert durch 'entkopplung an Phononen') %TODO entkoppelt? 
...
\item Die Master Gleichung beschreibt \textit{ausschließlich} irreversible Dynamiken, nicht aber die Bewegungsgleichung innerhalb des Systems $\tilde{H}$! %TODO image
...
\textit{Wir verdoppeln ein System sodass  %TODO image
\begin{align*}
\begin{rcases}
U_{ges}= 2U \\
V_{ges} = 2V \\
...
\item \textbf{Idee:} (falsch!) Wähle zufälligen Punkt im Einheitsquadrat und normiere. %TODO image
...
\end{itemize}
			...
			\item \textbf{ kleine Schritte um den alten Vektor $\vec{S}$ innerhalb eines Kegels} %TODO image kegel
			\begin{itemize}
...
%TODO images (2)
$1d$ Molekularphysik mit $\mathbf{F} + \mathbf{F}(t)$ kann nicht chaotisch werden! Ab $d=3$ kann eine Trajektorie in höhere Dimensionen ausweichen. Siehe bild %TODO : 
...
Wie definiert/bestimmet man die Dimension eines Objektes bzw. einer Punktmenge? \\ %TODO  image gitter
...
\textbf{Einfaches Beispiel:} (1-dim System granularer Materie (geht auch in 2,3D)) \\ %TODO image
...
\begin{itemize}
\item entweder abschneiden, falls Wechselwirkung hinreichend schnell abfällt (siehe Bild oben) %TODO reference
...
$N=80$, $1d$, $R=0$ (links getrieben, rechts reflektiert $v  \dashrightarrow -v$) %TODO image
...
%TODO %TODO %TODO Hier gehts nächstes mal weiter
...
ANHANG:
\includepdf[pages={1,2,3,4,5}]{Thouless_Uebergang_Nobelprice.pdf}  \label{siehe Dokument...} 
